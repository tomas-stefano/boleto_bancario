  ...
  module Core
    describe Inter do
      ...
      describe 'on validations' do
        ...
        it { is_expected.not_to have_valid(:carteira).when(nil, '', '1', '100') }
      ...
      describe '#conta_corrente' do
        context 'when have a value' do
          subject { Inter.new(conta_corrente: '12345') }
      ...
      describe '#numero_documento' do
        context 'when have a value' do
          subject { Inter.new(numero_documento: '1234') }
      ...
      describe '#codigo_banco' do
        it { expect(subject.codigo_banco).to eq '077' }
      end
      ...
      describe '#agencia_codigo_cedente' do
        subject { Inter.new(agencia: '0001', conta_corrente: '1234567890') }

        it { expect(subject.agencia_codigo_cedente).to eq '0001 / 1234567890-3' }
      end

      describe '#nosso_numero' do
        subject { Inter.new(numero_documento: '12345678') }

        it { expect(subject.nosso_numero).to eq '00012345678-9' }
      end

      describe '#to_partial_path' do
        it { expect(subject.to_partial_path).to eq 'boleto_bancario/inter' }
      end
    end
  end
end
